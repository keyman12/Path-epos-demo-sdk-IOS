...
//  Runs on a background queue so blocking stream I/O and TLS handshake don't freeze the UI.
//  Logs status to os_log (Diagnostics: Xcode console or Console.app, subsystem "SMTP").
//

import Foundation
import os

// MARK: - Config (same names as Dashboard .env)
struct SMTPConfig {
    var host: String
    var port: Int
    var useTLS: Bool
...
enum ReceiptEmailSender {
    private static let logger = Logger(subsystem: Bundle.main.bundleIdentifier ?? "PathEPOSDemo", category: "SMTP")
    ...
    private static func runSMTPOnRunLoop(
        ...
        // Let the run loop run until streams are open (connection established).
        ...
        /// Read one SMTP reply line by running the run loop and reading when bytes are available (required for TLS).
