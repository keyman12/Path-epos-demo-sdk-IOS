...
struct DeveloperDiagnosticsView: View {
    ...
    private var xcodeConsoleTips: String {
        """
        Xcode console: press Cmd+Shift+Y to show the debug area; choose “All Output” (not Errors Only). Run with the Debug scheme (Product → Scheme → Edit Scheme → Run → Build Configuration = Debug) or NSLog/print lines are stripped. Filter the console for PathTerminal. Logs below are always available in-app; the SDK does not print to stdout unless this app mirrors them (DEBUG only).
        """
    }
