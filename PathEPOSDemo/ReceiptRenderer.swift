...
//  No coordinate flip needed — UIImage and NSAttributedString draw naturally.
//
...
enum ReceiptRenderer {
    ...
    /// Max characters per line for 80mm receipt so text doesn't overflow or clip.
    private static let maxLineLength = 34

    /// Separator line at the width of the longest content line (maxLineLength chars).
    ...
    static func plainText(from receipt: FullReceipt) -> String {
        var lines: [String] = []
        // 1) TOP: Merchant name, address, order number, till, server
    ...
    /// PDF data (receipt-width) for print/email. Path Cafe logo left, all text black.
    static func pdfData(from receipt: FullReceipt) -> Data {
        ...
        return renderer.pdfData { ctx in
            ctx.beginPage()
            // UIGraphicsPDFRenderer uses UIKit coordinates (top-left origin, Y down).
            var drawY = margin
            // 1) Path Cafe logo — left justified
