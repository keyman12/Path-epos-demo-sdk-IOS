...
//  PathEPOSDemo
//
//  Type-erased wrapper for BLE or SDK terminal manager. Use this as the
//  EnvironmentObject so the app can switch between BLE and SDK via scheme.
//
...
/// Concrete type for @EnvironmentObject that forwards to either BLE or SDK manager.
@MainActor
final class AppTerminalManager: ObservableObject, TerminalConnectionManager {
    private let wrapped: any TerminalConnectionManager
    private var cancellables = Set<AnyCancellable>()
    
    init(ble: BLEUARTManager) {
        self.wrapped = ble
        subscribeToChanges(ble)
    }
